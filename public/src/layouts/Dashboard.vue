<template>
    <div class="bg-db"></div>
    <div class="db">
        <DashSidebar :profile="profile" :skin-count="skins.length" :cape-count="capes.length" :kit-count="kits.length"
            :friend-count="friends.length" :presence-class="presenceClass" @logout="handleLogout"
            @profile-updated="p => profile = p" />

        <div class="db__main">
            <!-- Tabs -->
            <nav class="db__tabs">
                <button v-for="t in TABS" :key="t.id" class="db__tab" :class="{ active: tab === t.id }"
                    @click="tab = t.id">
            ...
            <!-- Tab panels -->
            <DashProfile v-if="tab === 'profile'" :profile="profile" :skins="skins" :capes="capes" :tier-key="tierKey"
                @profile-updated="p => profile = p" />

            <DashCosmetics v-if="tab === 'cosmetics'" :skins="skins" :capes="capes" :kits="kits" :loading="loadingCosm"
                :tier-key="tierKey" @activate-skin="doActivateSkin" @activate-cape="doActivateCape"
                @activate-kit="doActivateKit" @open-upload-skin="showUploadSkin = true"
                @open-upload-cape="showUploadCape = true" @open-create-kit="showCreateKit = true" />

            <DashFriends v-if="tab === 'friends'" :friends="friends" :req-received="reqReceived" :req-sent="reqSent"
                :loading="loadingFriends" @accept="doAccept" @reject="doReject" />

            <DashPresence v-if="tab === 'presence'" :uuid="currentUser?.uuid ?? ''" :current-presence="currPres" />

            <DashSession v-if="tab === 'session'" :profile="profile" :tier-label="tierLabel" />
        </div>

        <!-- Modals -->
        <DashModals :show-skin="showUploadSkin" :show-cape="showUploadCape" :show-kit="showCreateKit" :skins="skins"
            :capes="capes" @close-skin="showUploadSkin = false" @close-cape="showUploadCape = false"
            @close-kit="showCreateKit = false" @skin-uploaded="loadCosm" @cape-uploaded="loadCosm"
